...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="app-shell">
    ...
    <aside ref={sidebarRef} role={menuOpen ? "dialog" : undefined} aria-modal={menuOpen || undefined} className={`sidebar ${menuOpen ? "sidebar-open" : ""}`} aria-label="Asosiy navigatsiya">
      <div className="sidebar-brand"><Link href="/" aria-label="ArmTv bosh sahifa" onClick={() => setMenuOpen(false)}><Brand/></Link><button className="icon-button mobile-sidebar-close" aria-label="Menyuni yopish" onClick={() => setMenuOpen(false)}><X size={22}/></button></div>
    ...
    <div className="main-shell">
      ...
      <footer className="site-footer"><div><Brand small/><p>Vatan, bilim va ilhom bir joyda.</p></div><span>© {new Date().getFullYear()} ArmTv. Loyiha namoyishi.</span><button onClick={() => setHelpOpen(true)}>Platforma haqida <ArrowUpRight size={13}/></button></footer>
    </div>
